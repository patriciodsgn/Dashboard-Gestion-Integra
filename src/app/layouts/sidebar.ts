...
export class SidebarComponent implements OnInit {
  ...
  getDirectionLink(direccion: any): string {
    // Obtener el valor de categoría y convertirlo a minúsculas
    const category = direccion.ValorCategoria?.toLowerCase();
    console.log('Procesando dirección:', {
      category,
      direccion
    });
    ...
    // Si existe una ruta mapeada, usarla
    if (routeMap[category]) {
      console.log('Ruta encontrada:', routeMap[category]);
      return routeMap[category];
    }
  
    // Si no hay mapeo, usar la ruta por defecto
    console.log('Usando ruta por defecto:', `/direccion/${direccion.codigo}`);
    return `/direccion/${direccion.codigo}`;
  }
}
